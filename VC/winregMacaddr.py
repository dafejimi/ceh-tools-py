...
def printNets():
    net = "SOFTWARE\Microsoft\Windows NT \CurrentVersion"+"\NetworksList\Signatures\Unmanaged"
...
def main():
    ...
    parser.add_option('-p', dest='userpassword', type='string', help='specify wigle password')
    (options, args) = parser.parse_args()
    username = options.username
    password = options.password
    if username == None or passwors == None:
        print(parser.usage)
        exit(0)
    else:
